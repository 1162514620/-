            ...
            try:
                while True:
                    if User.Name == '':
                        ...
                        break
                Msg = (User.Name, MsgItemName, ''.join(
                    [str(i) for i in MsgItem.GetRuntimeId()]))
...
class WeChat():
    ...
    def SendMsg(self, Name, msg, clear=True):
        '''向当前窗口发送消息
        msg : 要发送的消息
        clear : 是否清除当前已编辑内容
        '''
        self.EditMsg = self.UiaAPI.EditControl(Name=Name)
        self.UiaAPI.SwitchToThisWindow()
        if clear:
        ...
        def roll_to(who=who, RollTimes=RollTimes):
            for i in range(RollTimes):
                if who not in self.GetSessionList()[:-1]:
                    ...
                    time.sleep(0.5)
                    self.SessionList.ListItemControl(
    ...
    def GetPresentPosition(self, MsgItems):
        if (MsgItems.LocalizedControlType != '文本'):
            MsgItem = MsgItems.GetChildren()
            for each in MsgItem:
                self.GetPresentPosition(each)
        else:
            self.AllTextMsg.append(MsgItems.BoundingRectangle)
    ...
    def GetAllText(self, MsgItems):
        if (MsgItems.LocalizedControlType != '文本'):
            ...
            if (not (MsgItems.Name in self.AllTextMsg)):
        ...
        for MsgItem in MsgItems:
            if (MsgItem.Name == '[聊天记录]'):
                try:
                    ...
                    if (((self.AllTextMsg[0].Name.find('和') != -1) or (self.AllTextMsg[0].Name.find('与') != -1)) and (self.AllTextMsg[0].Name.find('的聊天记录') != -1)):
                        num = 0
                    else:
                        num = 1
                    位置 = self.AllTextMsg[0+num].BoundingRectangle
                    ...
                    self.AllTextMsg[0+num].Click(waitTime=0.1)
                    # 获得聊天记录实例
                    self.聊天记录窗口 = auto.WindowControl(ClassName='ChatRecordWnd')
                    self.聊天记录窗口.SetActive()
                    # 切换窗口
                    self.聊天记录窗口.SwitchToThisWindow()

                    try:
                        聊天记录列表 = self.聊天记录窗口.ListControl(
                        ...
                        for i in range(50):
                            self.获取聊天记录(聊天记录列表)
                            self.AllTextMsg = []
                            self.GetPresentPosition(聊天记录列表)
                            if (上一次位置记录 == self.AllTextMsg):
                                记录次数 += 1
                                if (记录次数 >= 2):
                                    break
                            else:
                                记录次数 = 0
                            上一次位置记录 = self.AllTextMsg
    ...
    try:
        ...
        while (True):
            ...
            if (Group != '返回'):
                ...
                dis.set_option("GroupChatListPartition",
                               "GroupChat"+str(i)+'Partition', '')
            else:
                break
        print("初始化内容结束")
    except:
        ...
        if (option == '1'):
            try:
                ...
                for wxql in GroupChatList:
                    ...
                    # 获取分割词，若没有则添加
                    Partition = dis.get_option(
                        'GroupChatListPartition', wxql+'Partition')
                    if (Partition == ''):
                        Partition = QlName+'>>分割线'
                        wx.SendMsg(QlName, Partition)
                        dis.set_option("GroupChatListPartition",
                                       wxql+'Partition', Partition)
                    ...
                    wx.SendMsg(QlName, '群聊'+QlName+'的消息已处理')
                ...
                for i in range(len(wxlist)):
                    for each in taobaolist:
                        if (wxlist[i] == each['订单编号']):
                            ...
                            xrsheet.cell(row=i+1, column=13,
                                         value=each['店铺名称'])
...
if __name__ == "__main__":
    # 调用函数获取CPU序列号
    # 设备ID = get_cpu_serial_number()
    # url = "http://110.40.208.208:2401/is_bound/"+设备ID
    # response = requests.get(url)
    # if (response.status_code == 200):
    #     response_json = response.json()
    #     if (response_json['message'] == '找不到该设备ID'):
    #         print('软件未激活，请输入激活码:')
    #         激活码 = input()
    #         if (len(激活码) == 27):
    #             url = "http://110.40.208.208:2401/bind"
    #             response = requests.post(
    #                 url, json={'code': 激活码, 'device_id': 设备ID})
    #             if (response.status_code == 200):
    #                 response_json = response.json()
    #                 print(response_json)
    #     elif (response_json['message'] == '激活码服务已过期'):
    #         print('软件服务时间已过期，请联系管理员。')
    #     elif (response_json['message'] == '设备ID已绑定激活码'):
    #         print('欢迎使用!')
